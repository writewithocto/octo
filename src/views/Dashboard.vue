<template>
  <div class="flex flex-col">
    <div class="flex flex-grow flex-shrink min-h-0">
      <TheLeftSidebar v-if="!mobile && showLeftSidebar" class="hidden w-72 bg-darker md:flex" />
      <div class="flex flex-col flex-grow flex-shrink min-h-0">
        <TheContent class="flex-grow flex-shrink min-h-0" />
      </div>
      <TheRightSidebar v-if="!mobile && showRightSidebar && currentDoc" class="hidden w-72 bg-darker md:flex" />
    </div>
  </div>
</template>
